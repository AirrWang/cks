...
public class BindNFCVipActivity extends Activity {

    private KyLoadingBuilder builder;
    private TextView tv_nfc_card;
    private GetNFCInfoBean.GetNFCDetailInfoBean body;
    private TextView tv_nfc_relname;
    private TextView tv_nfc_tel;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_bind_nfc_hui_yuan);
                ...
                if (data.length > 0) {
                    if (data[0] == 2) {
                        new Handler().postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                initToWriteCardTwo(array, 1, 10, (array.length + 1) / 4 + 1 - 1, (array.length + 1) % 4);
                            }
                        }, 380);
    ...
    private void initData() {
        ...
        String token = (String) SPUtils.get(BindNFCVipActivity.this, "token", "");
        int user_id = (int) SPUtils.get(BindNFCVipActivity.this, "user_id", 0);
        Call<GetNFCInfoBean> callBack = restApi.getnfcinfo(token, user_id + "");

        callBack.enqueue(new Callback<GetNFCInfoBean>() {
            @Override
            public void onResponse(Call<GetNFCInfoBean> call, Response<GetNFCInfoBean> response) {

                if (response.body() != null) {
                    if (response.body().getStatus().equals("0")) {
                        ToastUtils.showShort(BindNFCVipActivity.this, response.body().getMsg());
                        finish();
            ...
            @Override
            public void onFailure(Call<GetNFCInfoBean> call, Throwable t) {
                MoudleUtils.toChekWifi(BindNFCVipActivity.this);
                MoudleUtils.kyloadingDismiss(builder);//使登录按钮可按
    ...
    private void initUI() {
        final String card = body.getCard_no();
        MoudleUtils.textViewSetText(tv_nfc_card, card);
        MoudleUtils.textViewSetText(tv_nfc_relname, body.getRelname());
        MoudleUtils.textViewSetText(tv_nfc_tel, body.getTel());
                ...
                if (AppConfig.mBluetoothGatt != null && AppConfig.bluetoothGattServices != null) {
                    if (HomeFragment.nHealthFlag && HomeFragment.nSleepFlag) {
                        if (card != null && !card.equals("")) {
                            ...
                            initToWriteCard(array, 1, 10, (array.length + 1) / 4 + 1 - 1, (array.length + 1) % 4);
                        }
                    } else {
    ...
    private void initId() {
        ...
        tv_nfc_tel = (TextView) findViewById(R.id.tv_nfc_tel);
