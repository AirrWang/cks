...
public class BindNFCMjActivity extends Activity {

    private KyLoadingBuilder builder;
    private TextView tv_nfc_card;
    private GetnfcaccessBean.GetnfcaccessBeanInfo body;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_bind_nfc_mj);
                ...
                if (data.length > 0) {
                    if (data[0] == 2) {
                        new Handler().postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                initToWriteCardTwo(array, 11, 20, (array.length + 1) / 4 + 11 - 1, (array.length + 1) % 4);
                            }
                        }, 380);
                    }
                }
    ...
    private void initData() {
        ...
        String token = (String) SPUtils.get(BindNFCMjActivity.this, "token", "");
        int user_id = (int) SPUtils.get(BindNFCMjActivity.this, "user_id", 0);
        Call<GetnfcaccessBean> callBack = restApi.getnfcaccess(token, user_id);

        callBack.enqueue(new Callback<GetnfcaccessBean>() {
            @Override
            public void onResponse(Call<GetnfcaccessBean> call, Response<GetnfcaccessBean> response) {

                if (response.body() != null) {
                    if (response.body().getStatus().equals("0")) {
                        ToastUtils.showShort(BindNFCMjActivity.this, response.body().getMsg());
                        finish();
            ...
            @Override
            public void onFailure(Call<GetnfcaccessBean> call, Throwable t) {
                MoudleUtils.toChekWifi(BindNFCMjActivity.this);
                MoudleUtils.kyloadingDismiss(builder);
            }
        });
    ...
    synchronized void initToWriteNfc(int i2, int n, final char[] array, int nY) {
    ...
    private void initUI() {
        final String card = body.getAccesscard();
                ...
                if (AppConfig.mBluetoothGatt != null && AppConfig.bluetoothGattServices != null) {
                    if (HomeFragment.nHealthFlag && HomeFragment.nSleepFlag) {
                        if (card != null && !card.equals("")) {
                            ...
                            initToWriteCard(array, 11, 20, (array.length + 1) / 4 + 11 - 1, (array.length + 1) % 4);
                        }
